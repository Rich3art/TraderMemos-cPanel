...
describe("AccountNavPopover", () => {
  ...
  it("switches directly from one account to another", async () => {
    const user = userEvent.setup();
    useFilters.getState().setAccounts(["a1"]);
    wrap(<AccountNavPopover />);

    await user.click(screen.getByRole("button", { name: /Account: Live/i }));
    await user.click(await screen.findByRole("menuitemcheckbox", { name: /Paper USD/ }));

    expect(useFilters.getState().accountIds).toEqual(["a2"]);
  });

  it("keeps same-currency portfolio behavior when multiple accounts are selected", async () => {
    const user = userEvent.setup();
