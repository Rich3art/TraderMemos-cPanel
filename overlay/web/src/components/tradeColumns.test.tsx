...
describe("tradeColumns", () => {
  ...
  it("keeps FX prices precise and uses price change percent", () => {
    const fxTrade: Trade = {
      ...TRADE,
      id: "fx1",
      symbol: "USDCAD",
      ...
      direction: "short",
      qty_opened: 0.01,
      avg_entry_price: 1.37595,
      avg_exit_price: 1.3818,
      net_pnl: -67.77,
      return_pct: -0.43,
    };
    render(
      <DataTable
        columns={tradeColumns("ZAR", { onOpenFullPage: vi.fn<(...args: any[]) => any>() })}
        data={[fxTrade]}
        columnPinning={TRADE_COLUMN_PINNING}
      />,
    );
    expect(screen.getAllByText("1.37595")).toHaveLength(2);
    expect(screen.getAllByText("1.3818")).toHaveLength(2);
    expect(screen.getByText("-ZAR 67.77")).toBeInTheDocument();
