...
export function tradeListPrices(
  ...
  const convert: ConvertMoney =
    typeof fxRateOrConvert === "function"
      ? fxRateOrConvert
...
export function TradeListItem({
  ...
  usePrivacyMode();
  const convert: ConvertMoney =
    typeof fxRate === "function" ? fxRate : (value) => value * (fxRate ?? 1);
    ...
    >
      <ItemContent className="gap-1">
        <div className="flex items-start justify-between gap-3">
          ...
          >
            {trade.net_pnl != null
              ? fmtSignedMoney(convert(trade.net_pnl, trade.pnl_currency), currency, intlLocale())
              : "—"}
          </span>
        </div>
        ...
        <div className="flex items-baseline justify-between gap-3 text-[14px] tabular-nums">
          <span className="min-w-0 truncate text-foreground">
            {tradeListPrices(trade, currency, convert)}
