...
export function MobileTabBar() {
  ...
  const me = useMe();
  const tabItems = PRIMARY_NAV.filter(
    (item) => item.to !== "/reports" && canOpenRoute(me.data, item.to),
  );
  const split = Math.ceil(tabItems.length / 2);

  const navRef = useRef<HTMLElement>(null);
  const itemRefs = useRef(new Map<string, HTMLAnchorElement>());
  const [pill, setPill] = useState({ left: 0, top: 0, width: 0, height: 0, ready: false });

  const activeItem = tabItems.find((item) => isRouteActive(pathname, item.to));
  ...
  return (
    ...
    >
      ...
      >
        ...
        {tabItems.slice(0, split).map((item) => (
          <TabLink
            key={item.to}
            item={item}
