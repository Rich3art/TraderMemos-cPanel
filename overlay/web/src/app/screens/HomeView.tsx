...
export function HomeView({
  ...
  const sourceCurrencies = useMemo(() => tradeCurrencies(trades), [trades]);
  const moneyFx = useCurrencyConverter(currency, sourceCurrencies);
  const fxRate = 1;
  const displayTrades = useMemo(
    () => trades.map((trade) => convertTradeMoney(trade, currency, moneyFx.convert)),
    [trades, currency, moneyFx.convert],
  );
  const displaySummary = useMemo(
    () => summarizeTradesForDisplay(displayTrades, summary),
    [displayTrades, summary],
  );
  ...
  return (
    <Page className="min-h-[calc(100svh-52px)]">
      ...
      <div className="grid gap-4 lg:grid-cols-5 lg:grid-rows-[minmax(320px,auto)] lg:items-stretch">
        ...
        {displaySummary && !summaryLoading && !summaryError ? (
            ...
            {compact ? (
              <ItemGroup className="gap-2 px-4 pb-4">
                {recentTrades.map((trade) => (
                  <TradeListItem
                    key={trade.id}
                    trade={convertTradeMoney(trade, currency, moneyFx.convert)}
                    currency={currency}
                    fxRate={1}
                    showDate
