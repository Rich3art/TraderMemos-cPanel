...
function BrokerSetup({
  ...
  async function copyText(value: string, label: string) {
    try {
      ...
      toast.add({ title: "Copy failed", description: "Select the value and copy it manually." });
    }
  }
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,340px)]">
        <div className="flex flex-col gap-4">
          ...
          {broker.kind === "bridge" ? (
            <Card
              title="MetaTrader bridge"
              description="Run the bridge inside MT4 or MT5. It sends filled orders to this server with your access token."
            >
              <div className="flex flex-col gap-3">
                {error ? <p className="text-[12px] text-destructive">{error}</p> : null}
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div className="rounded-md bg-muted/60 px-3 py-2">
                      <div className="mb-1 flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground">
                        <Server size={12} strokeWidth={1.75} aria-hidden />
                        Server URL
                      </div>
                      <button
                        type="button"
                        onClick={() => void copyText(window.location.origin, "Server URL")}
                      ...
                      >
                        ...
                        <Copy size={12} strokeWidth={1.75} aria-hidden className="shrink-0" />
                      </button>
                    </div>
                    <div className="rounded-md bg-muted/60 px-3 py-2">
                      <div className="mb-1 flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground">
                      ...
                      <button
                        type="button"
                        onClick={() => void copyText(bridgeAccountId, "Account ID")}
                        className="flex w-full items-center justify-between gap-2 text-left text-[12px] font-medium text-foreground"
                      >
                        <span className="truncate">{bridgeAccountId}</span>
                        <Copy size={12} strokeWidth={1.75} aria-hidden className="shrink-0" />
                      </button>
                    </div>
                  </div>
                ) : (
                  <p className="rounded-md bg-muted/60 px-3 py-2 text-[12px] leading-relaxed text-muted-foreground">
                    Prepare the bridge first so MetaTrader gets the exact account ID to sync into.
                  </p>
                )}

                <div className="flex flex-wrap items-center gap-2">
                  <Button type="button" onClick={handlePrepareBridge} disabled={!canAct}>
                    {busy ? (
                      <>
                        <RefreshCw size={13} strokeWidth={1.5} className="animate-spin" />
                        Preparing…
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    disabled={!bridgeAccountId}
                    render={
                      bridgeAccountId ? (
                        <a href="/metatrader-bridge/TraderMemosSync.mq5" download />
                      ) : undefined
                    }
                  >
                  ...
                  <Button
                    ...
                    render={
                      bridgeAccountId ? (
                        <a href="/metatrader-bridge/TraderMemosSync.mq4" download />
                      ) : undefined
                    }
                  >
                    <Download size={13} strokeWidth={1.5} />
                ...
                <p className="text-[12px] leading-relaxed text-muted-foreground">
                  Create the token in Settings → API, then paste it into the Expert Advisor as
                  <span className="font-medium text-foreground"> TraderMemosToken</span>. Keep the
                  token private; revoke it from Settings if you stop using the bridge.
                </p>
              </div>
            </Card>
          ) : null}
