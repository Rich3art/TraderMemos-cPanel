...
function EditAccountButton({ account }: { account: Account }) {
  ...
  const [baseCurrency, setBaseCurrency] = useState(
    account.base_currency?.trim().toUpperCase() || "USD",
  );
  ...
  function handleSave() {
    ...
    update.mutate(
      { id: account.id, body: { name: name.trim(), broker, base_currency: currency } },
      {
        onSuccess: () => {
          setOpen(false);
          setError(null);
    ...
    <>
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <Field
            label="Base currency"
            htmlFor="account-detail-base-currency"
            description="Used for this account's balance and new trades. Existing trades keep their saved P&L currency."
          >
            <NativeSelect
              id="account-detail-base-currency"
              value={baseCurrency}
