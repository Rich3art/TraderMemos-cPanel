...
describe("CalendarView", () => {
  ...
  it("selecting a day in the week review drawer closes it and opens the day trades drawer", async () => {
    ...
    expect(screen.getByText("Day snapshot")).toBeInTheDocument();
  ...
  it("opens the selected day as a compact left-side panel with trades, notes, events, and full day action", async () => {
    const onOpenDayReview = vi.fn<(...args: any[]) => any>();
    const onNewTrade = vi.fn<(...args: any[]) => any>();
    const onNewNote = vi.fn<(...args: any[]) => any>();
    wrap(
      <CalendarView
        {...BASE}
        ...
        dayNotes={[
          {
            ...
            title: "London recap",
            body: "Waited for the pullback before entering.",
            symbols: [],
    ...
    expect(screen.getByText("London recap")).toBeInTheDocument();
    ...
    expect(onOpenDayReview).toHaveBeenCalledWith("2026-07-01");
