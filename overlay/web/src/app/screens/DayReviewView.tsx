...
function stripSessionPlanMeta(body: string): string {
...
export function DayReviewView({
  ...
  const sessionPlan = notes.find((n) => n.type === "session_plan");
  const journalNotes = notes.filter((n) => n.type !== "session_plan");
  const dailyLogs = journalNotes.filter((n) => n.type === "daily_log");
  const dayNotes = dailyLogs.length > 0 ? dailyLogs : journalNotes;
      ...
      <Card title="Session plan">
        {notesLoading ? (
          ...
          <div className="flex flex-col gap-2">
            <p className="m-0 text-[13px] leading-relaxed whitespace-pre-wrap text-muted-foreground">
              {noteExcerpt(stripSessionPlanMeta(sessionPlan.body)) || "No session plan details saved."}
            </p>
          </div>
        ) : (
          <p className="m-0 text-[13px] leading-relaxed text-muted-foreground">
            No session plan saved for this day.
          </p>
        )}
      </Card>
