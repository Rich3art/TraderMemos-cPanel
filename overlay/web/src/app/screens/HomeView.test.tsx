...
vi.mock("../../lib/hooks/useCurrencyConverter", () => ({
  useCurrencyConverter: (targetCurrency: string) => ({
    targetCurrency,
...
describe("HomeView", () => {
  it("renders the stats strip from the summary", () => {
    ...
    // Display money is now recomputed from the converted trades, not blindly
    // relabeled from the API's account-scope summary.
    expect(screen.getAllByText(/\+\$11\.39/).length).toBeGreaterThan(0);
  });
