...
describe("ConnectView", () => {
  ...
  it("prepares the MetaTrader bridge with the selected account id and download links", async () => {
    const user = userEvent.setup();
    listed.accounts = [account];
    createAccount.mockClear();
    renderConnect({ brokerKey: "metatrader" });

    await user.click(screen.getByRole("button", { name: /Prepare bridge/ }));

    expect(await screen.findByText("a1")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /MT5 bridge/ })).toHaveAttribute(
      "href",
      "/metatrader-bridge/TraderMemosSync.mq5",
    );
    expect(screen.getByRole("link", { name: /MT4 bridge/ })).toHaveAttribute(
      "href",
      "/metatrader-bridge/TraderMemosSync.mq4",
    );
    expect(createAccount).not.toHaveBeenCalled();
  });
