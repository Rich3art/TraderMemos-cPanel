...
export function validateEventsSearch(search: Record<string, unknown>): {
  ...
  const sector =
    typeof search.sector === "string" && CALENDAR_SECTORS.includes(search.sector as EconomicCalendarSector)
      ? (search.sector as EconomicCalendarSector)
      : undefined;
...
function EventsPage() {
  const { wk, sector, imp, ccy } = Route.useSearch();
  ...
  return (
    <EconomicEventsView
      ...
      sector={sector}
      onSectorChange={(next) =>
        void navigate({ to: "/events", search: (prev) => ({ ...prev, sector: next }) })
      }
      currencies={ccy}
