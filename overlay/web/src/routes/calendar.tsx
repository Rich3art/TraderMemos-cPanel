...
function CalendarPage() {
  ...
  const selectedDayFilters = useMemo(
    () =>
      selectedDay
        ? {
            from: normalizeFilterDate(selectedDay, "start", tz) ?? selectedDay,
            to: normalizeFilterDate(selectedDay, "end", tz) ?? selectedDay,
          }
        : null,
    [selectedDay, tz],
  );
  ...
  return (
    <>
      <CalendarView
        ...
        dayNotes={selectedDay ? (notesQ.data ?? []) : []}
        dayNotesLoading={Boolean(selectedDay) && notesQ.isLoading}
        dayNotesError={Boolean(selectedDay) && notesQ.isError}
        dayEvents={selectedDay ? (eventsQ.data ?? []) : []}
        dayEventsLoading={Boolean(selectedDay) && eventsQ.isLoading}
        dayEventsError={Boolean(selectedDay) && eventsQ.isError}
        currency={currency}
