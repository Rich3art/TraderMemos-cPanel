...
export const DISPLAY_CURRENCIES = CURRENCY_OPTIONS.map((currency) => currency.code);
...
function isDisplayCurrencyCode(value: string): value is DisplayCurrencyCode {
  return (DISPLAY_CURRENCIES as readonly string[]).includes(value.trim().toUpperCase());
}
      ...
      setDisplayCurrency: (displayCurrency) =>
        set({
          displayCurrency:
            displayCurrency == null
              ? null
              : isDisplayCurrencyCode(displayCurrency)
      ...
      merge: (persisted, current) => {
        ...
        if (raw && "displayCurrency" in raw) {
          ...
          displayCurrency =
            v === null || v === undefined
              ? null
              : typeof v === "string" && isDisplayCurrencyCode(v)
                ? v.trim().toUpperCase()
                : current.displayCurrency;
        } else if (raw?.currency && isDisplayCurrencyCode(raw.currency)) {
          displayCurrency = raw.currency.trim().toUpperCase();
        }
        const timezone =
