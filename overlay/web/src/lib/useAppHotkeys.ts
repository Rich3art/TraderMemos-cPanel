...
export function useAppHotkeys() {
  ...
  const openFromHotkey = (modal: "new-trade" | "new-setup" | "new-note" | "new-guide") =>
    guarded(() => {
      useUI.getState().setCommandOpen(false);
      openModal(modal);
    });
  ...
  useHotkeys(keys["action-new-guide"].keys, openFromHotkey("new-guide"), pageChord, [
    openModal,
    keys["action-new-guide"].keys,
  ]);
