...
describe("broker catalogue", () => {
  ...
  it("routes MetaTrader through the terminal bridge while preserving statement upload", () => {
    const mt = findBroker("metatrader");
    expect(mt?.kind).toBe("bridge");
    expect(mt?.formats).toMatch(/statement/i);
    expect(mt?.steps.join(" ")).toMatch(/WebRequest/i);
  });
});
