...
import { marketDefaultsForSymbol } from "./marketInference";
...
export function symbolTradeFromOcr(
  ...
  const inferred = marketDefaultsForSymbol(scoped.symbol || symbol);
  let market: string = inferred.market;
  ...
  return emptySymbolTrade({
    ...
    market,
    futuresPresetId:
      market === "future" || market === "futures" ? inferred.futuresPresetId : CUSTOM_PRESET_ID,
    multiplier:
      market === "option"
        ? "100"
        : market === "future"
