...
describe("display currency override", () => {
  ...
  it("accepts expanded display currencies", () => {
    useDisplayPrefs.getState().setDisplayCurrency("zar");
    expect(resolveDisplayCurrency("USD")).toBe("ZAR");
  });

  it("rejects unsupported display currency codes", () => {
    useDisplayPrefs.getState().setDisplayCurrency("NOPE");
