...
export const RISK_RULE_DEFS: readonly RiskRuleDef[] = [
  ...
  },
  {
    ...
    detail: "Stop when realized P&L drops this much from today's peak.",
    unit: "$",
    placeholder: "e.g. 300",
  },
  {
...
export function defaultRiskFormValues(rules?: RiskRules | null): RiskFormValues {
  return {
    ...
    maxDailyDrawdown: numOrEmpty(rules?.max_daily_drawdown),
...
export function riskFormToBody(value: RiskFormValues): RiskRules {
  return {
    ...
    max_daily_drawdown: parseOptionalAmount(value.maxDailyDrawdown),
...
export function validateRiskForm(value: RiskFormValues): string | undefined {
  return (
    ...
    validateOptionalAmountField(value.maxDailyDrawdown) ||
