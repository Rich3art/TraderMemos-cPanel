...
import { DISPLAY_CURRENCIES, useDisplayPrefs } from "@/lib/displayPrefs";
...
function isCurrencyCode(value: unknown): value is string {
  return (
    typeof value === "string" &&
    (DISPLAY_CURRENCIES as readonly string[]).includes(value.trim().toUpperCase())
  );
}
...
export function applyRemotePrefs(remote: SyncedPrefs): void {
  ...
  if ("displayCurrency" in remote) {
    ...
    else if (isCurrencyCode(v)) d.setDisplayCurrency(v.trim().toUpperCase());
  }
