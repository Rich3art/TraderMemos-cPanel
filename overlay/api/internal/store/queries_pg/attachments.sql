...
-- name: InsertSetupAttachment :one
INSERT INTO setup_attachments (id, user_id, setup_id, filename, content_type, size_bytes, storage_key)
VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *;

-- name: ListAttachmentsForSetup :many
SELECT * FROM setup_attachments WHERE setup_id = $1 AND user_id = $2 ORDER BY created_at;

-- name: GetSetupAttachment :one
SELECT * FROM setup_attachments WHERE id = $1 AND user_id = $2;

-- name: DeleteSetupAttachment :execrows
DELETE FROM setup_attachments WHERE id = $1 AND user_id = $2;
