...
SELECT * FROM setup_attachments WHERE setup_id = ? AND user_id = ? ORDER BY created_at;

-- name: GetSetupAttachment :one
SELECT * FROM setup_attachments WHERE id = ? AND user_id = ?;

-- name: DeleteSetupAttachment :execrows
DELETE FROM setup_attachments WHERE id = ? AND user_id = ?;
