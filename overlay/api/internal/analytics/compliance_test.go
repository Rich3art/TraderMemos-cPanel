...
func TestComplianceDailyDrawdownFromPeak(t *testing.T) {
	rules := ComplianceRules{MaxDailyDrawdown: 200}
	// Starts green, gives back 250 from the day's peak, then recovers green.
	trades := []ComplianceTrade{
		ct(1, 10, 300, 0),
		ct(1, 12, -250, 0),
		ct(1, 14, 100, 0),
	...
	rep := Compliance(trades, rules, time.UTC)
	if len(rep.Days) != 1 || !rep.Days[0].DrawdownBreach {
		t.Fatalf("expected daily drawdown breach, got %+v", rep.Days)
	}
