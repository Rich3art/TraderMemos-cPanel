...
type ComplianceRules struct {
	...
	// Positive peak-to-trough realized P&L drawdown allowed inside one day.
...
func (r ComplianceRules) configured() bool {
	return r.MaxRiskPerTrade > 0 || r.MaxDailyLoss > 0 || r.MaxTradesPerDay > 0 ||
		r.MaxDailyDrawdown > 0 || r.MaxConsecutiveLosses > 0
}
...
type ComplianceDay struct {
	...
	DrawdownBreach   bool    `json:"drawdown_breach"`
...
// ordered by close time) dips below -MaxDailyLoss at any point. Daily drawdown
// is scored separately as the day's worst peak-to-trough drop from realized
// P&L highs. Finishing green after breaking either intraday limit still counts
// as a breach. A trade violates the risk rule when its recorded initial risk
// exceeds MaxRiskPerTrade; trades with no recorded risk are reported
// separately rather than silently passed or failed. A day breaches the
// trade-count rule when more than MaxTradesPerDay trades close in it, and the
// loss-streak rule when a trade closes after the day already held
...
func Compliance(trades []ComplianceTrade, rules ComplianceRules, loc *time.Location) ComplianceReport {
	...
	for _, t := range sorted {
		...
		} else if rules.MaxDailyDrawdown > 0 {
			running[key] += t.NetPnl
		}
		if rules.MaxDailyDrawdown > 0 {
			if running[key] > dayPeak[key] {
				dayPeak[key] = running[key]
			}
			drawdown := dayPeak[key] - running[key]
			if drawdown > d.DailyDrawdown {
				d.DailyDrawdown = drawdown
			}
			if drawdown > rules.MaxDailyDrawdown {
				d.DrawdownBreach = true
			}
		}
	}
	...
	for _, key := range order {
		d := byDay[key]
		d.Compliant = d.RiskViolations == 0 && !d.DailyLossBreach && !d.DrawdownBreach && !d.TradeLimitBreach &&
			!d.LossStreakBreach
