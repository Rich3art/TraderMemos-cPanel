...
func TestCreateExecutionUsesMetaTraderAccountCurrency(t *testing.T) {
	s := testServer(t)
	tok := registerAndLogin(t, s, "mt-currency@x.com")
	acc := accountID(t, s, tok)

	buy := `{"account_id":"` + acc + `","symbol":"XAUUSD","instrument_type":"forex","side":"buy","quantity":1,"price":2500,"executed_at":"2026-01-01T10:00:00Z","multiplier":100,"details":{"source":"metatrader5","account_currency":"ZAR","broker_profit":"0"}}`
	sell := `{"account_id":"` + acc + `","symbol":"XAUUSD","instrument_type":"forex","side":"sell","quantity":1,"price":2499.25,"executed_at":"2026-01-01T11:00:00Z","multiplier":100,"details":{"source":"metatrader5","account_currency":"ZAR","broker_profit":"-1108.90"}}`
	...
	require.Equal(t, "ZAR", trade["pnl_currency"])
	require.Equal(t, -1108.9, trade["net_pnl"])
