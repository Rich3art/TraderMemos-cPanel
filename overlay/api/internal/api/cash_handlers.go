...
func (s *Server) cashRoutes(g *echo.Group) {
	...
	g.POST("/metatrader/cash-movements", s.handleCreateMetaTraderCash)
}
...
type createMetaTraderCashReq struct {
	AccountID  string    `json:"account_id"`
...
func (s *Server) handleCreateMetaTraderCash(c *echo.Context) error {
	var in createMetaTraderCashReq
	if err := c.Bind(&in); err != nil {
		return Fail(http.StatusBadRequest, "bad_request", "invalid body", nil)
	}
	...
	if in.AccountID == "" || in.ExternalID == "" || !validCashTypes[in.Type] {
		return Fail(http.StatusBadRequest, "bad_request", "account_id, external_id, and a valid type are required", nil)
	}
	if in.Currency == "" {
		in.Currency = "USD"
	}
	...
	if in.Amount == 0 {
		return Fail(http.StatusBadRequest, "bad_request", "amount must not be zero", nil)
	}
	uid := auth.UserID(c)
	if err := s.assertAccount(c.Request().Context(), uid, in.AccountID); err != nil {
		return Fail(http.StatusNotFound, "not_found", "account not found", nil)
	...
	importID := sql.NullString{String: "metatrader:" + in.ExternalID, Valid: true}
	if existing, err := s.deps.Store.GetCashTransactionByImportBatch(c.Request().Context(), store.GetCashTransactionByImportBatchParams{
		UserID: uid, AccountID: in.AccountID, ImportBatchID: importID,
	}); err == nil {
		return c.JSON(http.StatusOK, toCashDTO(existing))
	} else if !errors.Is(err, sql.ErrNoRows) {
		return Fail(http.StatusInternalServerError, "internal", "could not check cash transaction", nil)
	}

	tx, err := s.deps.Store.InsertCashTransaction(c.Request().Context(), store.InsertCashTransactionParams{
		ID: uuid.New().String(), UserID: uid, AccountID: in.AccountID,
		Type: in.Type, Amount: in.Amount, Currency: in.Currency,
		OccurredAt: in.OccurredAt, Note: in.Note, ImportBatchID: importID,
	})
	if err != nil {
		return Fail(http.StatusInternalServerError, "internal", "could not insert cash transaction", nil)
	}
