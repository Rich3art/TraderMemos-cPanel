...
func normalizePsychologyQuestions(questions []string) []string {
...
func encodePsychologyQuestions(questions []string) string {
	cleaned := normalizePsychologyQuestions(questions)
	b, err := json.Marshal(cleaned)
	if err != nil {
		return "[]"
	}
...
func parsePsychologyQuestions(raw string) []string {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return []string{}
	}
	...
	return normalizePsychologyQuestions(out)
}
...
func (s *Server) checklistRoutes(g *echo.Group) {
	...
	g.GET("/settings/psychology-questions", s.handleGetPsychologyQuestions)
	g.PUT("/settings/psychology-questions", s.handlePutPsychologyQuestions)
}
...
type psychologyQuestionsDTO struct {
...
func resolvePsychologyQuestions(row store.PsychologyQuestionSetting) psychologyQuestionsDTO {
	return psychologyQuestionsDTO{Questions: parsePsychologyQuestions(row.Questions)}
}
...
func (s *Server) handleGetPsychologyQuestions(c *echo.Context) error {
	row, err := s.deps.Store.GetPsychologyQuestions(c.Request().Context(), auth.UserID(c))
	if errors.Is(err, sql.ErrNoRows) {
		return c.JSON(http.StatusOK, psychologyQuestionsDTO{Questions: []string{}})
	}
	if err != nil {
		return Fail(http.StatusInternalServerError, "internal", "could not load psychology questions", nil)
	}
	return c.JSON(http.StatusOK, resolvePsychologyQuestions(row))
}

func (s *Server) handlePutPsychologyQuestions(c *echo.Context) error {
	var in psychologyQuestionsDTO
	if err := c.Bind(&in); err != nil {
		return Fail(http.StatusBadRequest, "bad_request", "invalid body", nil)
	}
	questions := normalizePsychologyQuestions(in.Questions)
	row, err := s.deps.Store.UpsertPsychologyQuestions(c.Request().Context(), store.UpsertPsychologyQuestionsParams{
		UserID:    auth.UserID(c),
		Questions: encodePsychologyQuestions(questions),
	})
	if err != nil {
		return Fail(http.StatusInternalServerError, "internal", "could not save psychology questions", nil)
	}
	return c.JSON(http.StatusOK, resolvePsychologyQuestions(row))
}
