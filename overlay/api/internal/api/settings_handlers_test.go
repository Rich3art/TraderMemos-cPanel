...
func TestRiskRulesRoundTrip(t *testing.T) {
	...
	body := `{"max_risk_per_trade":100,"max_daily_loss":300,"max_daily_drawdown":200,"max_open_risk":250,"default_account_risk_pct":1,"max_trades_per_day":5}`
	...
	require.Equal(t, 200.0, got["max_daily_drawdown"])
...
func TestPsychologyQuestionsRoundTrip(t *testing.T) {
	s := testServer(t)
	tok := registerAndLogin(t, s, "psychology@x.com")

	rec := do(s, http.MethodGet, "/api/v1/settings/psychology-questions", "", tok)
	require.Equal(t, http.StatusOK, rec.Code)
	var empty map[string]any
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &empty))
	require.Empty(t, empty["questions"])

	body := `{"questions":["Am I calm?","Am I following my plan?","Am I calm?","  "]}`
	rec = do(s, http.MethodPut, "/api/v1/settings/psychology-questions", body, tok)
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	...
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"Am I calm?", "Am I following my plan?"}, got.Questions)
}
