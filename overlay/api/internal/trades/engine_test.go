...
func TestBrokerProfitOverridesPriceFormula(t *testing.T) {
	open := ex("1", "buy", 1, 2500, "2026-01-01T10:00:00Z", 100)
	close := ex("2", "sell", 1, 2499.25, "2026-01-01T11:00:00Z", 100)
	close.BrokerProfit = f64(-1108.9)
	out := Group([]Execution{open, close})
	require.Len(t, out, 1)
	require.Equal(t, -1108.9, *out[0].NetPnl)
	require.Equal(t, -1108.9, *out[0].GrossPnl)
}
